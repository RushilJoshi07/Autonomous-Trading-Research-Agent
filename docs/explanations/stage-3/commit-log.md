...
## Stage 3 plan §10: Alembic baseline (stage close)

**Change:** Added `alembic>=1.13`, `alembic init migrations`, configured
`migrations/env.py` to read the DB URL from `data_pipeline.config.settings`
(prod default, `-x db=test` for test) rather than a hardcoded `alembic.ini`
URL. Autogenerated baseline revision came out empty (`pass`/`pass`) —
verified by reading the file before applying anything — since both
databases' schema already exactly matched `Base.metadata`. Stamped both prod
(4,164 real rows, unaffected) and test at head via `alembic stamp head`, not
`upgrade head`. Also populated `CLAUDE.md`'s previously-empty `## Commands`
section with the real setup sequence.

What is non-obvious: (1) `stamp head` was the deliberate choice over
`upgrade head` for a reason beyond these two databases — an empty migration
applied via `upgrade` to a genuinely fresh, empty database would create zero
tables, so the documented fresh-setup path still needs `init_db.py`'s
`create_all()` first, then `stamp head` — the exact same final command used
here, so there's one true sequence rather than two that could diverge. (2)
All three setup commands' idempotency was checked empirically, not assumed:
`createdb` confirmed to fail cleanly (not silently no-op) on a second run
against a throwaway scratch database; `create_all()` confirmed safe to
re-run directly against the real, populated prod database (exit 0, row
count unchanged); `stamp head` confirmed to be a clean no-op on a second run
against test. (3) `CLAUDE.md`'s `## Commands` section had been the literal
stub `"(To be filled in as the project grows.)"` since project init — no
setup doc existed anywhere in the repo, so this closes a real, pre-existing
gap the user explicitly flagged, not one Alembic introduced. 170/170 tests
unaffected.

---

## Stage 3 closed: Level-3 stage summary written

**Change:** Added `docs/explanations/stage-3/stage-3-summary.md` — the full
synthesis explainer for all 9 Stage 3 components plus the gate result, per
`CLAUDE.md`'s "after a stage passes its verification gate" rule and the
user's explicit request that this be a proper synthesis, not a quiet close.

What is non-obvious: this document deliberately does not re-walk any
component's code (each step file already owns that) — it synthesizes the
cross-component threads that only become visible looking back across the
whole stage: the two-tier registry as one decision spanning Components 2
and 8; "verify by execution" as the method that found every real bug in
this stage, from `bbands`' dead `std` to the gate script's RSI
investigation; provenance tracking growing from a nice-to-have into a
provable guarantee; the "prove the test isn't vacuous" discipline learned
once (Component 5) and then applied deliberately everywhere after; and the
bounded-disclosed-exception pattern appearing twice in structurally
identical form (the LLM boundary amendment, and `KNOWN_DEVIATIONS`). Honest
about what the gate does and does not prove — one ticker, one decade, one
disclosed deviation — rather than rounding up to a clean sweep.
